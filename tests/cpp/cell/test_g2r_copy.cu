    ...
    case tl::Layout::kColMajor:
      ...
      for (int i = 0; i < kWidth; ++i) {
        ...
        for (int j = 0; j < kHeight; ++j) {
          ...
          src_tile(j, i)(1, 0) = col * DstLayout::kColStride + row + 1;
          src_tile(j, i)(0, 1) = col * DstLayout::kColStride + row + 8;
          src_tile(j, i)(1, 1) = col * DstLayout::kColStride + row + 9;
          src_tile(j, i)(2, 0) = (col + 8) * DstLayout::kColStride + row;
          src_tile(j, i)(3, 0) = (col + 8) * DstLayout::kColStride + row + 1;
          src_tile(j, i)(2, 1) = (col + 8) * DstLayout::kColStride + row + 8;
          src_tile(j, i)(3, 1) = (col + 8) * DstLayout::kColStride + row + 9;
        }
      }
      break;
    default:
...
void run_load_g2r_test() {
  ...
  load_g2r<Element, kType, WarpLayout, RegLayout, GlobalLayout, BaseTile, kMode,
           kHeight, kWidth><<<1, 32 * kWarpSize>>>(d_src.data().get());
}
