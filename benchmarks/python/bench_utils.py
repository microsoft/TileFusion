...
This module provides functions for benchmarking PyTorch operations with various
configurations and measurement modes.
...
def do_bench(
    ...
    Returns:
        Union[float, list[float]]: Aggregated runtime in milliseconds
        or list of times
    """
    ...
    if _n_repeat > 0:
        n_repeat = _n_repeat
    start_event = [
        torch.cuda.Event(enable_timing=True) for _ in range(n_repeat)
    ]
    ...
    return cast(float, getattr(torch, return_mode)(times).item())
