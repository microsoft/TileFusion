...
struct STileIteratorPrettyPrinter {
  ...
  static HOST void print(std::ostream& out, const TileIterator& itr) {
    out << "SharedTileIterator {" << std::endl
        ...
        << TileIterator::sc0 << ", " << TileIterator::sc1 << ")" << std::endl
        << "}";
  }
};
...
struct SubTileLayoutCreator<TileLayout, kChunkRows, kChunkCols, false> {
  using type = tl::MatrixLayout<kChunkRows, kChunkCols, TileLayout::kRowStride,
                                TileLayout::kColStride, TileLayout::kType>;
};
...
class STileIterator {
 public:
  ...
  static_assert(Tile::kRows >= kChunkRows, "Tile::kRows must be >= kChunkRows");
  static_assert(Tile::kCols >= kChunkCols, "Tile::kCols must be >= kChunkCols");
  ...
  DEVICE auto operator()(int i) {
    ...
    using TileLayout = tl::MatrixLayout<kChunkRows, kChunkCols, kTileRowStride,
                                        kTileColStride>;
    using NewTile =
