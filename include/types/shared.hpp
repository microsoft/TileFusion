...
class SharedTile {
 public:
  ...
  using TileLayout = std::conditional_t<
      ...
      tl::MatrixLayout<kRows / kSwizzleRows, kCols / kSwizzleCols, kSwizzleRows,
                       kColStride * kSwizzleCols>>;
  ...
  DEVICE SharedTile(DType* data) : data_(data), layout_(Layout{}), offset_(0) {}
 ...
 private:
  ...
  DEVICE int swizzle_offset(int offset) {
    ...
    int in_swizzle_tile_offset = in_tile_layout_(in_tile_id.x, in_tile_id.y);
