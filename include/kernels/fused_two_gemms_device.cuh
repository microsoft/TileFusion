...
struct FusedTwoGemmsTraits {
  ...
  using SharedA =
      SharedTile<InType, tl::RowMajor<kTM, kTK>, kUseSwizzling, kSharedAccess>;
  ...
  using SharedB =
      SharedTile<InType, tl::ColMajor<kTK, kTN>, kUseSwizzling, kSharedAccess>;
  ...
  using SharedC =
      SharedTile<InType, tl::ColMajor<kTN, kTP>, kUseSwizzling, kSharedAccess>;
  ...
  using SharedD =
      SharedTile<InType, tl::RowMajor<kTM, kTP>, kUseSwizzling, kSharedAccess>;
  ...
  using RegDHalf = RegTile<BaseTileRowMajor<InType>, tl::RowMajor<kDMs, kDPs>>;
...
__device__ __forceinline__ void ke_fused_two_gemms(const InType* dA,
  ...
  const InType* gC_ptr = dC + blockIdx.z * (kN * kP) + blockIdx.y * (kTP * kN);
