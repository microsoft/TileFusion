...
#pragma unroll
      for (int j = 0; j < kColExec; ++j) {
        int tile_offset =
            global_offset + i * kSharedRowStride * BaseShape::kRows +
            j * BaseShape::kCols + lane_row * kSharedRowStride + lane_col;
        int offset =
            shared_tile.fetch_physical_offset(tile_offset) - iterator_offset;
...
#pragma unroll
      for (int j = 0; j < kRowExec; ++j) {
        int tile_offset =
            global_offset + i * kSharedColStride * BaseShape::kCols +
            j * BaseShape::kRows + lane_col * kSharedColStride + lane_row;
        int offset =
            shared_tile.fetch_physical_offset(tile_offset) - iterator_offset;
...
#pragma unroll
          for (int n = 0; n < StoreMat::kSegCols; ++n) {
            col =
                StoreMat::kElemPerSeg * (lane_col + n * StoreMat::kThreadCols);
            int in_tile_offset = row * Shared::kRowStride + col;
            int offset = tile_offset + in_tile_offset;
            int swizzled_offset = shared_tile.fetch_physical_offset(offset);

            const PackedType* src_ptr =
                reinterpret_cast<const PackedType*>(src(i, j).data());
            PackedType* dst_ptr = reinterpret_cast<PackedType*>(dst);
 ...
 private:
  ...
  using MmaAtom =
      compute::MmaAtom<__half, __half, __half, compute::MMA_ATOM_16x16x16>;
  using BaseShape = MmaAtom::BaseTile;
  using PackedType = typename Packing<DType, StoreMat::kElemPerSeg>::PackedType;
...
#pragma unroll
        for (int m = 0; m < StoreMat::kSegRows; ++m) {
          row = StoreMat::kElemPerSeg * (lane_row + m * StoreMat::kThreadRows);
#pragma unroll
          for (int n = 0; n < StoreMat::kSegCols; ++n) {
...
struct RegToSharedStorer {
  ...
  DEVICE void operator()(const Reg& src, Shared& dst_) {
    ...
    static_assert(Shared::kType == Reg::kType,
