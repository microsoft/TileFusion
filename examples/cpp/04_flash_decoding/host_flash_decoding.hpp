...
void host_flash_decoding(int kM, int kN, int kK, int kP, const __half* Q,
                         const __half* K, const __half* V, __half* O,
                         __half* acc, __half* exp_values, __half* cur_row_max,
                         __half* prev_row_max, __half* new_row_max,
                         __half* prev_norm_vec, __half* new_norm_vec,
                         __half* prev_sums, __half* cur_sums,
                         __half* new_sums) {
    const __half* q_ = Q;
    const __half* k_ = K;
    const __half* v_ = V;
    ...
    for (int i = 0; i < kM; ++i) {
        for (int j = 0; j < kP; ++j) {
            o_[i * kP + j] = (o_[i * kP + j] * prev_sums[i] * prev_norm_vec[i] +
