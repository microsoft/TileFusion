...
void run_test() {
  ...
  using Config =
      KeGemmTraits<InType, AccType, WholeShape, CtaTileShape, kRK, WarpLayout>;
  auto kernel = &gemm<InType, AccType, kM, kN, kK, kTM, kTN, kTK,
  ...
  if (passed) {
    ...
    std::cout << "cuBLAS\ttilefusion\tRatio" << std::endl;
