...
void flash_attention(const torch::Tensor& Q, const torch::Tensor& K,
  ...
  kernel_name_ss << "flash_attention_kernel_" << in_type << "_" << acc_type
                 ...
                 << "_" << hidden_qk << "_" << hidden_v << "_" << tile_length_q
                 << "_" << tile_length_kv << "_" << tile_hidden_qk << "_"
                 << tile_hidden_v;
  std::string kernel_name = kernel_name_ss.str();

  std::string kernel_wrapper_src = generate_kernel_wrapper(
      kernel_name, in_type, acc_type, out_type, length_q, length_kv, hidden_qk,
      hidden_v, tile_length_q, tile_length_kv, tile_hidden_qk, tile_hidden_v,
      tl::num_rows<WarpLayout>, tl::num_cols<WarpLayout>, softmax_scale,
      causal);
  ...
  CUfunction kernel = jit.get_or_compile_kernel(kernel_name, kernel_wrapper_src,
