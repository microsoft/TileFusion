...
extern "C" __global__ void gemm_kernel_)"
     ...
     << pipeline_level << R"((const )" << in_type << R"(* A, const )" << in_type
     << R"(* B, )" << acc_type << R"(* C) {)";
  ss << std::endl;
  if (pipeline_level == 0) {
...
void gemm(const torch::Tensor& A, const torch::Tensor& B, torch::Tensor& C,
  ...
  std::string kernel_name =
      ...
      std::to_string(tm) + "_" + std::to_string(tn) + "_" + std::to_string(tk) +
      "_" + std::to_string(num_stages) + "_" + std::to_string(pipeline_level);
